...
IndexBuilder::IndexBuilder(const booktagsvector& bt, const ParserOpts& po)
    : parser(po), book_tags(bt) {
}
...
forwardmap IndexBuilder::build_all_forward(
    std::vector<str>& ram_book) noexcept {
  ...
  for (const auto& [id, doc] : external) {
    ...
    for (int row_attr = 0; row_attr < new_ir.size() - 1; ++row_attr) {
      ...
      for (const auto& ngram_node : doc_ngrams.ngrams) {
        build_inverted(
            new_ir.at(row_attr), ngram_node, row_attr, std::atoi(id.c_str()));
      }
    }
  }
  return new_ir;
...
TextIndexWriter::TextIndexWriter(
    const booktagsvector& bt,
    const IdxWriterOpts& iwo)
    : book_tags(bt), options(iwo) {
}
TextIndexWriter::TextIndexWriter(const booktagsvector& bt, cint p_l, cint h_l)
    : book_tags(bt) {
  options.part_length = p_l;
  options.hash_length = h_l;
}

str TextIndexWriter::name_to_hash(cstr& name) const noexcept {
  return picosha2::hash256_hex_string(name).substr(0, options.hash_length);
}
void TextIndexWriter::write_all_forward(const forwardmap& docindex, cstr& path)
  ...
  // запись кол-ва документов в отдельный файл
  std::ofstream filecount(path + "all_docs.dat");
  filecount << doc_counter;
  filecount.close();
}
...
void TextIndexWriter::write_all_inverted(
  ...
  ASSERT(indexresult.empty(), "БД книг не найдена. Невозможна запись!");
  ...
  for (auto it = book_tags.begin() + 1; it != book_tags.end(); ++it) {
    ...
    for (const auto& [idx, entr] : indexresult.at(attrpos++)) {
      cstr part = name_to_hash(cstr(idx, 0, options.part_length));
      if (part != prev_part) {
        ngram_fileset.close();
